...
5. Когда уместнее использовать `type`, а когда `interface` в TypeScript? Приведите примеры?
	...
	- `interface` может расширять `type`, если тот раскрывается в объектный тип (не союз). Пересечения (`A & B`) допустимы, если результат — валидный объектный тип.
...
16. Как получить тип ключей (`keyof`) и тип значений (`T[keyof T]`) объекта в TypeScript?
	...
	- Замечание: индексные сигнатуры влияют на `keyof` — строковая индексная сигнатура даёт `string | number` (т.к. JS приводит числовые ключи к строкам). `symbol` появляется в `keyof` только при явной `[key: symbol]` сигнатуре.
