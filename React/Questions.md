...
- Что делает `useSyncExternalStore` и в каких случаях он необходим?
	...
	- Eго основная задача — гарантировать, что подписка и чтение состояния извне происходит **синхронно** и **без рассинхронизации**, особенно важно для Concurrent Mode и SSR.
- Какие аргументы принимает useSyncExternalStore и что он возвращает?
	...
	- **getSnapshot** — функция, которая синхронно возвращает актуальный снимок (snapshot) состояния из стора. React вызывает её при рендере, чтобы получить текущее состояни
...
- Почему нельзя использовать `useState`/`useEffect` для подписки на внешний store?
...
- Какие проблемы решает useSyncExternalStore?
	- Обеспечивает, что React читает актуальное состояние _синхронно_ во время рендера
...
- Как работает `getSnapshot` и когда он должен быть стабилен?
...
- Что произойдёт, если `getSnapshot` возвращает новое значение при каждом вызове?
	- приведёт к бесконечному циклу ререндеров
- Как работает подписка — синхронно или асинхронно?
...
- В чём разница между `subscribe` и `getSnapshot` по таймингу?
...
- Как работает SSR-поддержка через `useSyncExternalStoreWithSelector`?
...
- Что происходит, если snapshot меняется во время рендера?
...
- Какие ошибки возникают при нестабильном snapshot или unsubscribe?
	...
	- Неправильный unsubscribe привядищй к утечке
...
- Почему `useInsertionEffect` нужен для библиотек CSS-in-JS ?
...
- Что произойдёт, если внутри `useInsertionEffect` сделать `setState`?
...
- Почему нельзя читать DOM внутри `useInsertionEffect`?
...
- Как `useInsertionEffect` влияет на порядок вставки CSS и приоритет specificity?
...
- Что произойдёт, если `useInsertionEffect` недоступен (например, SSR)?
...
3. Можно ли в `useImperativeHandle` возвращать функции, которые используют stale props/state?
	1. Да, можно, **если правильно указать зависимости**:
4. В каких случаях стоит избегать `useImperativeHandle` вообще?
...
5. Что произойдёт, если ref в `forwardRef` null или изменится тип?
...
### useDebugValue
- Как работает `useDebugValue` и влияет ли он на продакшн-бандл?
...
- Чем `useActionState` отличается от обычного `useState` + `useEffect`?
...
- Какаие аргмуенты принимает useActionState?
	- - `actionFn`: асинхронная функция или Server Action, вызываемая при отправке формы. Получает первым аргументом текущий (или предыдущий) state, затем formData.
...
- Как `useActionState` помогает обрабатывать формы и асинхронные действия?
...
- Что делает `action(prevState, formData)` и как использовать `formData` внутри?
...
- Как работает взаимодействие с `Server Actions`?
	- При использовании с Server Actions `useActionState` позволяет получить результат выполнения серверной функции и сразу обновить локальное состояние.
...
- Что происходит, если action возвращает ошибку — влияет ли это на UI?
...
- Можно ли вызвать `setState` внутри `action` напрямую?
...
- Как сбрасывается state, если отправка формы отменена или изменилась?
...
- Как `useActionState` влияет на retry, Suspense и optimistic UI?
	...
	- Поддержка Suspense и progressive hydration реализуется через серверные механизмы React и Server Actions.
...
- как синхронизировать состояния state у useActionState с пропсами
