...
###  **1. Concurrent Rendering и Automatic Batching**
- Конкурентный рендеринг в React 18+
- Влияние конкурентного рендеринга на жизненный цикл UI
- Автоматическая пакетная обработка обновлений (automatic batching)
---
### **2. SSR и гидрация**
- Streaming SSR в React 18
- Server Components (на концептуальном уровне, без хаков)
- Синхронизация серверного и клиентского рендеринга (гидрация)
---
### **3. Рендеринг и оптимизация**
- Правильное использование `key` в списках
- Reconciliation: как React определяет необходимость обновления UI
---
### **4. Code splitting и ленивый рендеринг**
- `React.lazy` и `Suspense` для динамического импорта компонентов    
- Варианты fallback UI и вложенность `Suspense`    
---
###  **5. Жизненный цикл функциональных компонентов (без хуков)**
- Этапы монтирования, обновления и размонтирования
- Поведение компонентов в разных режимах рендера
---
###  **6. Контроль UI через props и состояние**
- Однонаправленный поток данных
- Подъём состояния (lifting state up)
- Контролируемые и неконтролируемые компоненты (в т.ч. формы)
- Использование `children` и паттерн Render Props
---
###  **7. Архитектура компонентов**
- Контейнерные и презентационные компоненты (Smart/Dumb)
- Использование `React.Fragment` и ключей в фрагментах
---
### **8. Работа с Error Boundaries**
- Обработка ошибок в React 18+ (через классовые компоненты, но с фокусом на современное поведение)
- Альтернативы Error Boundaries при отсутствии классов
---
###  **9. ReactDOM.createPortal**
- Использование порталов для модальных окон, тултипов и других UI-элементов
- Особенности событий и фокуса в порталах
---
###  **10. Управление фокусом и доступность (a11y)**
- Создание доступного UI с помощью стандартных атрибутов и API React
- Управление фокусом при открытии модалок и при навигации
---
###  **11. Работа с формами**
- Контролируемые и неконтролируемые компоненты
- Обработка событий, `onChange`, `onSubmit`    
- Простейшая валидация без сторонних библиотек
---
###  **12. Анимации без сторонних библиотек**
- CSS-переходы, классы и keyframe-анимации
- Использование `React Transition Group` (если только в рамках React API)
---
### **13. Event Delegation и Synthetic Events**
- Как работает `SyntheticEvent`
- Отличия от нативных событий
- Особенности системы событий React 18+
---
### **14. Strict Mode и его влияние**
- Поведение Strict Mode в React 18
- Зачем повторно вызываются компоненты
- Как это помогает в разработке
---
###  **16. Context API (без хуков)**
- `<MyContext.Provider>` и `<MyContext.Consumer>`
- Использование без `useContext`
- Проблемы производительности при частых обновлениях
---
### **17. JSX под капотом**
- Преобразование JSX в `React.createElement`
- Что такое `ReactElement`
- Отличие между функцией-компонентом и результатом её вызова
---
###  **18. UI как чистая функция от данных**
- Компоненты без состояния
- Рендеринг только через props
- Минимизация побочных эффектов и логики внутри компонентов
