...
- **Reflow (перерасчёт макета/компоновки)** и **repaint (перерисовка)**: Частые изменения стилей могут вызывать перерасчёт геометрии элементов и перерисовки, что может повлиять на производительность.
