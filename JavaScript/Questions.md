...
4. Что такое область видимости (scope) и какие типы области видимости существуют в JavaScript?
	1. Область видимости - это контекст, в рамках которого определяются и становятся доступными переменные, функции и другие объекты
	2. типы областей видимости
...
8. Что такое Symbol и для чего они используются в JavaScript?
	1. уникальность и неизменность примитивный тип данных 
	2. уникальные ключи для объектов
	3. создание скрытых свойств
...
10. В чем разница между null и undefined?
	1. null для явного указания пустоты или обнуление
	2. undefined когда не присвоено значение или у объекта отсутствует свойство
...
12. В чём разница между Number.isNaN() и isNaN() в JavaScript, и какой предпочтительнее использовать?
	1. isNaN() - пытается преобразовать переданный аргумент в число перед проверкой
	2. Number.isNaN() безя преобразовагтя проверяет переданный аргумент
...
16. В чём разница между глубоким (deep) и поверхностным (shallow) копированием объектов в JavaScript?
	1. Глубокое копирование копирует все уровни вложенности так что изменения в копии **не затрагивают** оригинальный объект.
	2. Поверхностное копирование копирует только верхний уровень объекта, при этом вложенные объекты и массивы копируются по ссылке и ссылки на вложенные структуры сохраняются.
17. Что такое Map в JavaScript, чем он отличается от обычного объекта и от WeakMap?
...
18. Что такое Set в JavaScript, для каких задач он используется и чем отличается от WeakSet?
	1. Set это  объект который представляет собой коллекцию уникальных значений любого типа.
	2. WeakSet хранит только объекты
	3. У WeakSet нет методов для итерации (forEach, keys, values и т.д.), так как элементы могут удаляться динамически.
	4. WeakSet **не предотвращает сборку мусора**. Если объект больше не доступен в коде, он удаляется из WeakSet автоматически.
...
20. Что такое функции в JavaScript и для чего они используются?
	1. функция - это блок кода, который выполняет определённую задачу, и её можно многократно вызывать в разных частях кода.
...
26. Что такое Immediately Invoked Function Expression (IIFE) и для чего она используется?
	1. IIFE - это функция, которая определяется и немедленно вызывается. Она используется для создания изолированной области видимости
...
28. Что такое функции обратного вызова (callback) в JavaScript и зачем они нужны?
	1. это функции, которые передаются в качестве аргумента другой функции и вызываются внутри этой функции после выполнения определённого действия или при наступлении определённого события.
...
32. Что такое объект arguments в JavaScript и в каких способах объявления он недоступен? Почему?
	1. это встроенный в функции объект, который содержит массивоподобный список всех аргументов, переданных в функцию
	2. не поддерживается в arrow function, лучше использовать rest-параметры (остаточный оператор)
...
34. Что представляет собой ключевое слово this и как определяется его контекст в разных ситуациях?
    ...
    1. В методе объекта.
	    1. на контекст объекта
    1. В функции-конструкторе.
	    1. на контекст объекта созданный этой функцией оператором new
    1. В стрелочных функциях.
	    1. на контекст родителя
...
36. В чем разница между методами call(), apply() и bind() в JavaScript, и когда использовать каждый из них?
	1. Методы call(), apply() и bind() в JavaScript используются для управления контекстом вызова функции
	2. call() - вызывает функцию немедленно, с указанным значением this и переданными аргументами по отдельности.
	3. apply() - вызывает функцию немедленно, с указанным значением this и переданными аргументами в виде массива.
	4. bind() возвращает новую функцию с указанным значением this и фиксированными аргументами (если они переданы), в отличии от других методов функция не вызывается сразу и может быть вызвана в дальнейшем уже с изменым контекстом
...
40. Что такое очереди задач: macrotasks и microtasks, и как они влияют на порядок выполнения кода?
	1. Когда асинхронная задача попадает в **Call Stack**, она передается во внешнюю систему, которая после завершения добавляет её callback в macroTasks Queue или microTasks Queue. Когда Call Stack становится пустым, сначала обрабатываются задачи из macroTasks Queue, а затем из microTasks Queue
...
58. Чем отличается копирование массива с помощью slice, оператора spread и Array.from()?
	1. slice - позволяет копировать часть массива (подмассив) с указанием границ индексов.
	2. spread-оператора - поддерживает объединение массивов и добавление новых элементов в процессе копирования.
	3. Array.from() - работает с массивоподобными объектами (например, arguments или DOM-коллекциями) и позволяет преобразовывать элементы при копировании.
...
62. Что такое CSSOM и чем он отличается от DOM?
	1. CSS Object Model - это объектная модель CSS, которая представляет стили CSS в виде структуры дерева. CSSOM создается браузером при разборе CSS-стилей (как встроенных, так и внешних). Она позволяет JavaScript взаимодействовать с CSS — например, получать или изменять стили элементов на странице.
...
68. Что такое BOM (Browser Object Model) и какие возможности он предоставляет?
	1. Browser Object Model -  интерфейс, предоставляемый браузером для взаимодействия с окном браузера и его функциями
...
71. Как браузер рендерит документ: этапы от получения байтов до отображения?
	...
	10. Compositing: 
	    ...
	    1. Логика композитинга может включать отрисовку слоёв в правильном порядке, обработку прозрачности, тени и других визуальных эффектов.
	    2. Завершается отправкой кадра на GPU для отображения.